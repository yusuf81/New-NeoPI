...
class Test:
    ...
    # block and only return the highest (or lowest). this achieves finer
    # granularity.
    def blockCalculate(self, blocksize, data, filename):
        ...
        if(self.highIsBad == True):
            self.results.append(
                {"filename": filename, "value": maxEntropy, "position": pos})
            return {"value": maxEntropy, "position": pos}
        else:
            self.results.append(
                {"filename": filename, "value": minEntropy, "position": pos})
    ...
    def flagAlarm(self):
        ...
        flagList = []
        for res in self.results:
            # current value deviates more than a threshold amount of standard
            # deviation
            if dist(res["value"], self.mean) > (DEVIATION_THRESH)*self.stddev:
                # check that the deviation is in the correct "direction"
                # for example, we don't care when a file has very low entropy,
                # we only care when it's very high
                if (self.highIsBad and res["value"] > self.mean) or (not self.highIsBad and res["value"] < self.mean):
                    percentage = dist(
                        res["value"], self.mean) / self.stddev if self.stddev > 0 else float("inf")
...
class LanguageIC(Test):
    ...
    def calculate_IC(self):
        ...
        try:
            ic_total = float(total)/(self.total_char_count *
...
class Entropy(Test):
    ...
    def calculate(self, data, filename):
        ...
        entropy = 0
        for x in range(256):
            p_x = float(self.stripped_data.count(chr(x))) / \
                len(self.stripped_data)
            if p_x > 0:
...
class SignatureNasty(Test):
    ...
    def calculate(self, data, filename):
        ...
        valid_regex = re.compile(
            '(eval\(|file_put_contents|base64_decode|python_eval|exec\(|passthru|popen|proc_open|pcntl|assert\(|system\(|shell)', re.I)
...
class CharacterFreq(Test):
    ...
    def calculate(self, data, filename):
        if False:
            print "calculating frequecy for file: {}".format(filename)
        self.stripped_data = data.replace(' ', '')
        ...
        for wordsize in range(1, maxWordSize):
            if False:
                print "checking wordsize = {}".format(wordsize)
            for y in range(len(self.stripped_data) - (wordsize)):
                word = self.stripped_data[y:y+wordsize]
                numOccurences = self.stripped_data.count(
                    word) - 1  # -1 so that it doesn't count itself
                proportion = float(numOccurences) / \
                    (len(self.stripped_data) / (wordsize+1))
                if False:
                    print "checking for word = {}, found {} occurences with proportion = {}".format(word, numOccurences, proportion)
                if ((proportion) > highestoccurence[wordsize]):
                    highestoccurence[wordsize] = proportion
            if False:
                print "for wordsize of {}, the highest is {}".format(wordsize, highestoccurence[wordsize])
    ...
    def weightaverage(self, highoccurence):
        ...
        if False:
            print "numerator / sum = {}/{}".format(numerator, mySum)
        WA = float(numerator)/mySum
        if False:
            print "weightaverage is: {}".format(WA)
        return WA
...
if __name__ == "__main__":
   ...
   """
    ...
    # Instantiate the Generator Class used for searching, opening, and reading
    # files
    ...
    for data, filename in locator.search_file_path(args, valid_regex):
        if data:
            ...
            if (options.ignore_unicode == False or fileAsciiHighRatio < .1):
                for test in tests:
                    if options.block_mode:
                        calculated_value = test.blockCalculate(
                            int(options.block_mode), data, filename)
                    else:
                    ...
                    # possible optimization: move this into its own "for t in
                    # tests" loop?
                    if not options.block_mode:
    ...
    for test in tests:
        if (options.alarm_mode):
            ...
            for file in test.results:
                rank_list[file["filename"]] = rank_list.setdefault(
                    file["filename"], 0) + file["rank"]
